...
### PPO: Handling Stale Data

Now back to PPO. We have advantages from the value function. The second key idea in PPO is **clipping**, but to understand why it exists, we first need to understand a practical problem: **data reuse**.

**Why we reuse data:**

Generating responses is expensive. You have to run the model autoregressively, token by token, which is slow. So instead of generating fresh responses for every single gradient update, people typically generate a batch and then do multiple updates on it:

```python
# Generate responses (expensive - run model token by token)
responses = policy_v1.generate(prompts)
rewards = reward_model.score(responses)
advantages = compute_advantages(rewards, value_function)

# Do multiple gradient updates on this same batch (cheap)
for epoch in range(4):
    loss = compute_ppo_loss(policy, responses, advantages)
    loss.backward()
    optimizer.step()  # policy changes each time!
```

After the first `optimizer.step()`, we have policy_v2. But we're still learning from responses that policy_v1 generated. After the second step, policy_v3 is learning from policy_v1's responses. The data is becoming "stale" or "off-policy."

**Why stale data is a problem:**

The policy gradient formula assumes the responses came from your current policy. If response X was generated by policy_v1 but you're updating policy_v3, the gradient is biased. Maybe policy_v1 thought X had 10% probability, but policy_v3 now thinks it has 40% probability. Treating X as a "typical" sample from the current policy is wrong.

**What PPO's ratio measures:**

The ratio tells us how much the policy's opinion of a response has changed:

```python
ratio = P_current(response) / P_old(response)
```

- ratio ≈ 1.0 → policies agree, data is fresh, gradient is trustworthy
- ratio = 2.0 → current policy thinks this is 2x more likely than when it was generated
- ratio = 0.5 → current policy thinks this is half as likely

**What clipping does:**

```python
def ppo_loss(model, old_log_probs, prompt, response, advantage, epsilon=0.2):
    """
    PPO's clipped surrogate objective.

    The key insight: if the policy has changed too much since generating
    this response, stop learning from it (the data is too stale).
    """
    # Compute log prob under current model
    log_prob_new = compute_log_probability(model, prompt, response)

    # old_log_probs were stored when we generated the response
    # (we don't need to keep the old model around, just the log probs)
    ratio = math.exp(log_prob_new - old_log_probs)

    # Unclipped objective: ratio * advantage
    unclipped = ratio * advantage

    # Clipped objective: if ratio is outside [0.8, 1.2], clip it
...
If the ratio goes outside [1-ε, 1+ε], we clip it. This effectively says: "This response is too stale. The policy has changed too much since this was generated. Stop learning from it."

**The clipping logic in detail:**

When the advantage is positive (good response), we want to increase its probability. But if `ratio > 1.2`, the policy has *already* increased this response's probability a lot since generating it. Clipping says "you've learned enough from this example, move on."

When the advantage is negative (bad response), we want to decrease its probability. But if `ratio < 0.8`, the policy has *already* decreased it a lot. Clipping says "you've learned enough, move on."

The minimum operation ensures we always take the more conservative option. The intuition: be cautious with stale data.
...
>
> **Clipping** = "Stop learning from stale data"
>
> When you reuse generated responses for multiple gradient updates, the data becomes "off-policy" (it came from an older version of the model). The ratio `current_prob / old_prob` measures how stale the data is. If ratio is outside [0.8, 1.2], the data is too stale to learn from reliably. Clipping stops the gradient.
>
> **KL penalty** = "Don't drift too far from where you started"
>
> It's a tether to the original SFT model. Even if you generate fresh on-policy data every step, after 10,000 steps you could end up somewhere crazy (reward hacking). The KL penalty keeps pulling you back toward the reference.
>
> ```python
> # Clipping: data freshness check (is this sample still representative?)
> ratio = current_prob / prob_when_generated
> if ratio outside [0.8, 1.2]: stop learning from this sample
>
> # KL penalty: global drift check (how far from original model?)
> kl = log_prob_policy - log_prob_sft  # how different from SFT?
> adjusted_reward = reward - 0.01 * kl  # penalize drift
> ```
>
> **Key distinction:**
> - Clipping compares current policy to the policy that *generated this specific batch* (could be from 3 gradient steps ago)
> - KL penalty compares current policy to the *original SFT model* (fixed reference from the start of training)
>
> **When each matters:**
> - Without clipping: learning from stale data gives biased gradients, training becomes unstable
> - Without KL: model slowly drifts into reward hacking (finds degenerate outputs that score high on reward model but aren't actually good)
>
> **If you're strictly on-policy** (fresh data every update, like NanoChat), you don't need clipping at all; the ratio is always 1.0. But you might still want a KL penalty if you're worried about reward hacking.
